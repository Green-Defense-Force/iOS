//
//  PloggingCameraViewController.swift
//  GreenDefenseForce
//
//  Created by 이완재 on 12/12/23.
//

import UIKit
import AVFoundation

class PloggingCameraViewController: UIViewController, AVCapturePhotoCaptureDelegate {
    ...
    lazy var subtitleLabel: UILabel = {
        let label = UILabel()
        label.text = "플로깅 종료 후, 쓰레기 봉투 인증하기"
        label.textColor = .white
    ...
    lazy var backBtn: UIButton = {
        ...
        btn.addTarget(self, action: #selector(backToPloggingMap), for: .touchUpInside)
        return btn
    }()
    ...
    func photoOutput(_ output: AVCapturePhotoOutput, didFinishProcessingPhoto photo: AVCapturePhoto, error: Error?) {
        ...
        okBtn.addTarget(self, action: #selector(goToPloggingHome), for: .touchUpInside)
    ...
    // 촬영 후 모달에서 동작: 확인
    @objc func goToPloggingHome() {
        let ploggingVC = PloggingViewController()
        navigationController?.pushViewController(ploggingVC, animated: false)
